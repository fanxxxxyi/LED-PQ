...
def get_args():
    ...
    parser.add_argument('--resume', type=str, default='/home/fanxiaoyi/LED-PQ/ckpt//resnet_56_CIFAR10.t7',help='load the model from the specified checkpoint')
    parser.add_argument("--datapath", type=str, default='/home/fanxiaoyi/LED-PQ/model/data', help='Path toward the dataset that is used for this experiment')
    ...
    parser.add_argument("--lub", type=str, default='/home/fanxiaoyi/LED-PQ/log_30/resnet_56_ea_min.data', help='The affine transformations')
...
class LEDPQ:
    def __init__(self, dataset, datapath, model, pruner, rank_type='', batch_size=128, lr=1e-3, safeguard=0, global_random_rank=False, lub='', weight_decay=0, device=None):
        self.device = device
    ...
    def learn_ranking_ea(self, name, model_desc, tau_hat, long_ft, target):
        ...
        torch.save(self.pruner.model, os.path.join('./ckpt_{}'.format(args.prune_away), '{}_bestarch_init.pt'.format(args.savename)))
...
if __name__ == "__main__":
    ...
    # NOTE GRFP
    ledpq = LEDPQ(args.dataset, args.datapath, basemodel, args.pruner, args.rank_type, args.batch_size, args.lr, safeguard=args.safeguard, global_random_rank=args.global_random_rank, lub=args.lub, weight_decay= args.weight_decay, device=device)
    ...
    if args.min_lub:
        ledpq.learn_ranking_ea(args.savename, args.resume, args.tau_hat, args.long_ft, (1-(args.prune_away)/100.))
    else:
        ledpq.model_compress(args.savename, args.long_ft, (1-(args.prune_away)/100.))
